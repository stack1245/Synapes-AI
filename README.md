...
첫 진입 시 기존 세션이 없으면 프론트엔드가 자동으로 첫 오답 세션을 만들고 온보딩 메시지를 표시합니다. AI가 추천한 conceptId가 응답되면 개념 트리도 해당 노드로 부드럽게 포커스 이동하며, 현재 노드부터 최상위 부모 노드까지의 학습 경로 전체를 함께 하이라이트합니다.
...
프론트엔드는 라이트/다크 모드 토글을 지원하며, 사용자의 선택은 localStorage에 저장됩니다. 테마가 바뀌면 오른쪽 Cytoscape 개념 트리도 색상과 가독성이 실시간으로 함께 갱신되며, 경로 하이라이트 색상도 라이트/다크 모드에 맞춰 선명하게 조정됩니다.
